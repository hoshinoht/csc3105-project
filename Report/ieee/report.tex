...
\textit{1) CNN encoder:} A multi-scale first convolutional stage (parallel kernels 3/7/15) is followed by two residual Conv1D blocks (kernels 5 and 3) with max-pooling. This progressively maps the 1016-point CIR to a sequence of 127 feature vectors of dimension 128.
...
A key advantage of the CNN-Transformer is interpretability through attention weight visualization. Fig.~\ref{fig:attention} shows a representative attention pattern for sample CIR waveforms. Qualitatively, elevated attention often appears near first-arrival and multipath regions, offering suggestive evidence about which CIR segments influence predictions and complementing the feature-importance analysis of classical models.
