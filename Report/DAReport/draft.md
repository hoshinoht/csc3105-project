...
The following evaluation metrics are used across the different analysis components:

**Classification Evaluation:**

- **Confusion Matrix**: Displays True Positives, True Negatives, False Positives, and False Negatives, enabling detailed inspection of per-class error patterns (e.g., missed NLOS detections vs. false NLOS alarms).
- **ROC Curve and AUC**: The Receiver Operating Characteristic curve plots True Positive Rate against False Positive Rate across all classification thresholds. The Area Under the Curve (AUC) provides a single threshold-independent summary of discriminative quality, making it the primary cross-model comparison metric.
- **Precision-Recall Curve**: Complements ROC analysis by focusing on the trade-off between precision (fraction of predicted NLOS that are truly NLOS) and recall (fraction of actual NLOS correctly detected), which is particularly informative under class imbalance.
- **Accuracy**: Overall fraction of correct predictions. Reported alongside AUC but interpreted cautiously due to the 25/75 class imbalance in the expanded dataset.
- **Weighted F1 Score**: Used as the optimization target in GridSearchCV hyperparameter tuning, balancing precision and recall across both classes with class-frequency weighting.

**Regression Evaluation:**

- **RMSE (Root Mean Squared Error)**: Measures the average magnitude of prediction errors in meters, penalizing large errors more heavily than small ones.
- **MAE (Mean Absolute Error)**: Average absolute prediction error in meters, providing a more interpretable central-tendency error measure.
- **R² (Coefficient of Determination)**: Fraction of target variance explained by the model. $R^2 = 1$ indicates perfect prediction; $R^2 = 0$ indicates the model is no better than predicting the mean.

**Clustering Evaluation:**

- **Silhouette Score**: Measures how similar each point is to its own cluster compared to the nearest neighboring cluster, ranging from -1 (poor) to +1 (well-separated clusters).
- **Adjusted Rand Index (ARI)**: A label-permutation-invariant measure of agreement between cluster assignments and true labels, corrected for chance. ARI = 0 indicates random-level agreement; ARI = 1 indicates perfect agreement.
